...
nm -D "$OUT" 2>/dev/null | grep -E 'daed_(abi_version|solve_dense|solve_dense_ws|solve_dense_work_n|set_fail|dispatch|copy_f64)' || \
  nm "$OUT" 2>/dev/null | grep -E 'daed_(abi_version|solve_dense|solve_dense_ws|solve_dense_work_n|set_fail|dispatch|copy_f64)' || true
